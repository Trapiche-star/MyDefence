...
namespace MyDefence
{
    public class LevelSelect : MonoBehaviour
    {
        ...
        public RectTransform contectnsRect;
        ...
        private void Start()
        {
            ...
            if(scrollHeight > 0)
            {
                //자동 스크롤량 : 0~4: 0, 5~9: 110+/, 10~14: (110+7)*2
                ...
                //Debug.Log($"clearLeveHeight :{clearLeveHeight}");
            }
...
2. 로드 시점
 :레벨 설렉트 씬 시작할때 {보통은 게임 실행하자 마자}

3. 게임 로드시 체크사항
 : 저장 파일 유무 체크
파일이 없으면 - 유저 데이터를 초기화, 저장하여 세이브 파일을 만든다 
파일이 있으면 - 유저 데이터 파일에 저장한 데이터로 초기화

4. 게임 세이브시 체크사항
- 레벨 클리어 데이터는 저장된 데이터와  저장할 데이터를 비요한 후 저장한다
...
PlayerPrefs.GetFloa(KeyName)           //KeyName 이름으로 저장된 Value 가져오기   
